function [new_tau,new_resid] = par_update_tau(old_tau,theta,old_resid,kappa,sigmasq,delta, ...
    ...
    if par == true
        ...
        parfor p = 1:reg.num_reg_used
            ...
            [xp,yp,old_residp,old_taup,thetap,old_tau_neighbor,regp,smartp] = par_preprocess_tau(x,y,p,old_resid,theta,i,j,old_tau,reg,smart,kf,const);
        ...
        for p = 1:reg.num_reg_used
            ...
            [old_tau(p),old_resid(:,p)] = par_update_tau_pixel(xp,yp,old_residp,old_taup,old_tau_neighbor,thetap,kappa,sigmasq,delta,Method,...
            regp,smartp,ExtCroSect,CompSSA,kf,add_limit,const);        
