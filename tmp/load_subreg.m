function rad = load_subreg(Date,Path,Orbit,Block,const)
    
    Orbit = num2str(Orbit,'%06d');
    Path = num2str(Path,'%03d');

    % Load MI1B2T data, make averages and conversions
    rad = NaN *ones(const.XDim_r1100, const.YDim_r1100, const.Band_Dim, const.Cam_Dim);
    ...
    for cam = 1:const.Cam_Dim
        ...
        for band = 1:const.Band_Dim
            ...
            rad(:,:,band,cam) = pi*radiance*sun_distance_au{1}^2/std_solar_wgted_height{1};
        end
    end
    ...
    for ii = 1:const.XDim_r1100
        for jj = 1:const.YDim_r1100
            for cam = 1:const.Cam_Dim
                rho = squeeze(rad(ii, jj, :, cam));
                if all(~isnan(rho))
                    rad(ii,jj,:,cam) = const.Config_spectral_corr_matrix*rho;
                end
            end
        end
    end
    ...
    for band = 1:const.Band_Dim
        for cam = 1:const.Cam_Dim
            ...
            rad(:, :, band, cam) = rad(:, :, band, cam) .* kron(c, ones(const.RegSize*const.RegScale));
        end
    end
